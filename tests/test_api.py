...
def test_post_sem_regiao_e_rejeitado(client, headers_escrita):
    corpo = _payload()
    del corpo["regiao"]
    r = client.post("/medicoes", json=corpo, headers=headers_escrita)
    assert r.status_code == 422


def test_post_com_regiao_vazia_e_rejeitado(client, headers_escrita):
    r = client.post("/medicoes", json=_payload(regiao=""), headers=headers_escrita)
    assert r.status_code == 422


def test_post_com_altura_negativa_e_rejeitado(client, headers_escrita):
    r = client.post("/medicoes", json=_payload(altura=-1), headers=headers_escrita)
    assert r.status_code == 422


def test_post_com_nivel_invalido_e_rejeitado(client, headers_escrita):
    r = client.post(
        "/medicoes", json=_payload(risco="ALTISSIMA"), headers=headers_escrita
    )
    assert r.status_code == 422
...
def test_get_com_chave_de_leitura_funciona(client, headers_escrita, headers_leitura):
    client.post("/medicoes", json=_payload(), headers=headers_escrita)
...
def test_chave_de_escrita_tambem_le(client, headers_escrita):
    ...
    client.post("/medicoes", json=_payload(), headers=headers_escrita)
    r = client.get("/medicoes", headers=headers_escrita)
    assert r.status_code == 200


# --- GET /medicoes -----------------------------------------------------------
def test_get_filtra_por_regiao(client, headers_escrita, headers_leitura):
    client.post("/medicoes", json=_payload(regiao="norte"), headers=headers_escrita)
    client.post("/medicoes", json=_payload(regiao="sul"), headers=headers_escrita)
    r = client.get("/medicoes", params={"regiao": "sul"}, headers=headers_leitura)
    assert r.status_code == 200
    corpo = r.json()
    assert len(corpo) == 1
...
def test_get_ordena_do_mais_recente_para_o_mais_antigo(
    ...
    client.post("/medicoes", json=_payload(altura=1.0), headers=headers_escrita)
    client.post("/medicoes", json=_payload(altura=2.0), headers=headers_escrita)
    r = client.get("/medicoes", headers=headers_leitura)
...
def test_get_respeita_limit(client, headers_escrita, headers_leitura):
    for _ in range(3):
        client.post("/medicoes", json=_payload(), headers=headers_escrita)
    r = client.get("/medicoes", params={"limit": 2}, headers=headers_leitura)
    assert len(r.json()) == 2
...
def test_get_por_id_retorna_a_medicao(client, headers_escrita, headers_leitura):
    novo = client.post(
        "/medicoes", json=_payload(), headers=headers_escrita
    ).json()
...
def test_get_por_id_inexistente_retorna_404(client, headers_leitura):
    r = client.get("/medicoes/9999", headers=headers_leitura)
